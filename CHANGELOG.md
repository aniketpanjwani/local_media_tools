...
### Added
- **Page-by-page CLI access for scraped data**: `cli_web.py list-pages` and `read-page`
  - `list-pages` shows all scraped pages with index, source, and title
  - `read-page --source "Name" --index N` outputs one page's markdown
  - Prevents Claude from reading large JSON files (33k+ tokens) directly
  - Enables incremental processing without token limit issues

### Changed
- Updated `research-web-extract.md` to use page-by-page iteration instead of bulk JSON
